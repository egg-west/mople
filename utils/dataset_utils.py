...
def get_one_w_dataset(
    ...
    **kwargs,
    ...
    Arguments:
        ...
        obj_id: the objective index that this dataset is
    """
    assert mode == "rm" and dataset_name in ["anthropic_rlhf_helpful", "anthropic_rlhf_harmless"]
    data_path = data_path or conf.cache_dir
    dataset_name = dataset_name.lower()

    if dataset_name == "anthropic_rlhf_helpful":
        train, eval = load_anthropic_rlhf_helpful(n_obj, obj_id)
    elif dataset_name == "anthropic_rlhf_harmless":
        train, eval = load_anthropic_rlhf_harmless(n_obj, obj_id)
    else:
        raise NotImplementedError("Dataset but oasst_export not implemented")
    """
    if dataset_name in QA_DATASETS:
