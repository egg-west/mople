...
from dataset.formatting import DatasetEntrySft, Role, Utterance
...
def load_oasst_export(
    ...
    for tree in tree_iter:
        ...
        def leaf_filter(thread: list[ExportMessageNode]) -> bool:
            if mode == "sft":
                ...
                # fixed from the originl Open-Assistant repo
    ...
    def process_thread(thread: list[ExportMessageNode]):
        if mode == "sft":
            ...
            replies = [r for r in thread[-1].replies if r.role == "assistant" and ('toxicity' in r.labels.keys())]
            print(f"toxicity value: {replies[0].labels['toxicity'].value}")
